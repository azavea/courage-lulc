...
function sampleYear(year, fc) {
  ...
  //Map.addLayer(l8.clip(jordanClip).select('evi4'), eviVis, 'jordan evi4 '+year, false);
  //Map.addLayer(l8.clip(jordanClip).select('evi3'), eviVis, 'jordan evi3 '+year, false);
  //Map.addLayer(l8.clip(jordanClip).select('evi2'), eviVis, 'jordan evi2 '+year, false);
  //Map.addLayer(l8.clip(jordanClip).select('evi1'), eviVis, 'jordan evi1 '+year, false);
  Map.addLayer(l8.clip(jordanClip), rgbVis, 'jordan rgb '+year, false);
...
// ====================================================================
var mergedFC = urban.merge(other);
//print(mergedFC, 'mergedFC')
Export.table.toDrive({
  collection: mergedFC,
  description:'jordan-urban-binary',
  fileFormat: 'KML'
});
...
previewYear(trained, 2018, jordanClip)
previewYear(trained, 2017, jordanClip)
previewYear(trained, 2016, jordanClip)
previewYear(trained, 2015, jordanClip)
previewYear(trained, 2014, jordanClip)
previewYear(trained, 2013, jordanClip)
...
// Useful for regions too large to classify
var TILESCALE = 4

var sums = []
// classify 2010 - 2012 (L7)
for (var i = 2010; i <= 2012; i++) {
  var l7classified = yearMosaicL7(i).clip(jordanClip).classify(trainedL7).byte();
  Export.image.toDrive({
    image: l7classified,
    description: "jordan-urban-"+i,
    region: jordanBounds,
    scale: 30,
    maxPixels: 1e13
  });
...
for (var i = 2013; i <= 2018; i++) {
  var l8classified = yearMosaic(i).clip(jordanClip).classify(trained).byte();
  Export.image.toDrive({
    image: l8classified,
    description: "jordan-urban-"+i,
    region: jordanBounds,
...
// classify 2019
var classified2019 = mosaic2019().clip(jordanClip).classify(trained).byte();
Export.image.toDrive({
  image: classified2019,
  description: "jordan-urban-"+2019,
  region: jordanBounds,
  scale: 30,
  maxPixels: 1e13
});
...
for (var i = 0; i < sums.length; i++) {
  ...
  Export.table.toDrive({
    collection: sums[i],
    description:'jordan-urban-'+years[i],
    fileFormat: 'CSV'
  });
}

// https://code.earthengine.google.com/5ccbcc87e7b7c64fb88c5da989782c1d